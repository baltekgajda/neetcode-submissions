...
class Codec {
    ...
    fun deserialize(data: String): TreeNode? {
        ...
        fun deserialize(startIndex: Int): Pair<TreeNode?, Int> {
            ...
            val (left, nextIndex) = deserialize(index+1)
            val (right, returnIndex) = deserialize(nextIndex)
            node.left = left
            node.right = right
            return node to returnIndex
        }

        return deserialize(0).first
    }
}
